...
} = require("../services/accountMerge.service");
  ...
  new GoogleStrategy(
    {
      ...
      callbackURL: `${(process.env.BASE_URL || "http://localhost:5000").replace(/\/$/, "")}/auth/google/callback`,
      proxy: true,
    },
    async (accessToken, refreshToken, profile, done) => {
      ...
      try {
        ...
        const result = await linkSocialAccountToExisting(email, "google", id, {
          name: displayName,
          avatar,
  ...
  new GitHubStrategy(
    {
      ...
      callbackURL: `${(process.env.BASE_URL || "http://localhost:5000").replace(/\/$/, "")}/auth/github/callback`,
      proxy: true,
    },
    async (accessToken, refreshToken, profile, done) => {
      ...
      if (!email && accessToken) {
        try {
          ...
          if (Array.isArray(fetchedEmails)) {
            ...
            const primaryEmail =
              fetchedEmails.find((e) => e.primary && e.verified) ||
      ...
      if (!email) {
        ...
        console.warn(
          `Could not fetch real email for ${username}, using fallback: ${email}`,
        );
      }
      ...
      let avatar = photos?.[0]?.value || "";
      if (avatar)
        avatar = avatar.replace(/[?&]v=\d+/g, "").replace(/&s=\d+/g, "");
      ...
      try {
        ...
        // Step 2: Fall back to email-based lookup (for account merging)
        const result = await linkSocialAccountToExisting(email, "github", id, {
